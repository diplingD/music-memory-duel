...
public class ReducerTests
{
    ...
    public void SequenceSubmitted_InComposing_MovesToSolvingAndStartsRound()
    {
        ...
        Assert.Equal("p0", next.CurrentRound!.CreatorId);
    ...
    public void AnswerSubmitted_WhenEveryoneAnswered_ClosesRoundImmediately()
    {
        ...
        // creator confirms with the exact same sequence, then the only solver answers correctly
        var (afterCreator, creatorEffects) = Reducer.Reduce(
            afterSubmit, new AnswerSubmitted("p0", roundId, notes), DateTime.UtcNow);
        Assert.Empty(creatorEffects); // still waiting on the solver

        var (afterSolver, solverEffects) = Reducer.Reduce(
            afterCreator, new AnswerSubmitted("p1", roundId, notes), DateTime.UtcNow);
        ...
        Assert.True(roundEnded.CreatorConfirmed);
    ...
    [Fact]
    public void ResultDisplayFinished_InRoundResult_ReturnsToComposing()
    {
        var state = LobbyWith(2) with { Phase = Phase.RoundResult };
        var now = new DateTime(2026, 1, 1, 12, 0, 0, DateTimeKind.Utc);
